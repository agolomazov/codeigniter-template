...
## 4. Использование шаблонизаторов [Smarty](http://www.smarty.net/) и [Twig](http://twig.sensiolabs.org/)

Для использования шаблонизаторов Twig или Smarty (по умолчанию используется Twig) необходимо выполнить следующие действия:

1. Перейти в папку `path/to/application/config/`
2. В папке откройте файл my_config.php
3. Для выбора шаблонизатора по умолчанию необходимо изменить настройку `$config['default_template']` - значения могут быть следующие:
    * smarty
    * twig
    
4. В контроллере необходимо вызвать 2 метода:<br /><br />
    4.1 ***parent::setToData($key, $value)*** - метод, который записывает данные страницы в закрытую переменную. 
    4.1.1 ***$key*** - имя переменной, которая будет доступна в шаблоне.
    4.1.2 ***$value*** - значение переменной в шаблоне
    <br /><br />
    ***Например***:<br />
    ```
    parent::setToData('name', 'Admin')
    ```
    <br /><br />
    В шаблонизаторе ***Twig*** мы можем получить доступ к переменной следующим образом:
    ```
    {{name}}
    ```
    <br /><br />
    В шаблонизаторе ***Smarty*** мы можем получить доступ к переменной следующим образом:
    ```
    {$data.name}
    ```
    <br /><br />
    4.2 ***parent::render($template)*** - метод рендеринга шаблона. Параметр ***$template*** - имя шаблона